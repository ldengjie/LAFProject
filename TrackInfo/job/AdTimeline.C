...
void AdTimeline()
{
        ...
        for( int i1=0 ; i1<30 ; i1++ )
        {
            ...
            */
            ...
            //label->Draw();
	        TH2F* h2=new TH2F("charge","charge",24,0,24,8,0,8);
	        ...
	        for( int i=0 ; i<8 ; i++ )
	        {
	            for( int j=0 ; j<24 ; j++ )
	            {
	                if(charge[i][j]!=0) h2->Fill(j,i,(int(charge[i][j])+float(hitcount[i][j]+1)/10));
	            }
