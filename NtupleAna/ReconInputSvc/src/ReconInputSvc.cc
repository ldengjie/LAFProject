...
DECLARE_SERVICE(ReconInputSvc);

ReconInputSvc::ReconInputSvc(const std::string& name)
    : SvcBase(name),
      m_entry(-1),
      m_evt(0),
      m_ridxHandler(0)
{
    m_name = "ReconInputSvc";  //to avoid naming confusion
...
ReconInputSvc::~ReconInputSvc()
{
}

bool ReconInputSvc::initialize()
{
...
EvtObject* ReconInputSvc::next(int nth)
{
    PhyEvent* res = 0;
    m_entry += nth;
    if ( m_ridxHandler == 0 ) {
...
const std::string& ReconInputSvc::fname(int num)
{
    return m_files.at(num);
}

void ReconInputSvc::parse_files()
{
