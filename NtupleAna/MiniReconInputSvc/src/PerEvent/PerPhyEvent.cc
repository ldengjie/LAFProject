      ...
      m_files(infiles)
{
	...
	std::string msg("MiniReconInputSvc: unknown preferredEnergy ");
	msg += preferredEnergy;
...
DataSec* PerPhyEvent::getObj(const std::string& path, PhyEvent* pEvt)
{
    ...
    if ( it != m_topo.end() ) {
	obj = it->second->ToTran(pEvt->m_entry);
    }
    else {
...
Int_t PerPhyEvent::GetEntry(Long64_t entry)
{
    ...
    fCalibStatsChain->GetEntry(entry);
    return fRecReadoutChain->GetEntry(entry);
}
