...
LafMgr::LafMgr(char* optionfile)
    : m_nEvt(0),
      m_evtMax(-1),
      m_name("LafMain")
{
    LogInfo << "Welcome to the dayabay Lightweight Analysis Framework" << std::endl;
    ...
    OptionParser::setOption(name(),      "LogLevel", LafLog::LogLevel);
    ...
    if ( isvc.empty() ) {
	throw LafException(name()+".InputSvc is not set properly!");
    }
    if ( isvc != "NONE" ) {
	loadDll(isvc);
...
void LafMgr::loadDll(const std::string& dll)
{
