...
BetaSpec::BetaSpec(string name, int z, int a, int charge, string dir, bool useHuber,double qmax)
{
...
void BetaSpec::LoadSpectra(string filename)
{
    ...
    if( Qmax==0.0 ) Qmax = Q[0]; // by default
    MakeBetaSpectra();
}
...
void BetaSpec::Draw()
{
    ...
    TH1F *hTotal = new TH1F(hname, (isotope + " Spectra").c_str(), maxBins, 0, maxBins*binWidth);
    ...
    //h->SetLineColor(kBlue);
        ...
        //h->Draw("same");
        //}
    TFile f("IsoTheoreticalSpec.root", "update");
    hTotal->Write();
    f.Close();
}
