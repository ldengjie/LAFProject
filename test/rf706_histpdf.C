...
void rf706_histpdf()
{
  ...
  RooGaussian p("p","p",x,RooConst(7),RooConst(2.5)) ;
  RooGaussian p0("p0","p0",x,RooConst(12),RooConst(2.5)) ;
  ...
  x.setBins(100) ;
  RooDataSet* data2 = p.generate(x,10000000) ;
  ...
  RooRealVar coe("coe","coe",0.5);
  //RooAddPdf sum("sum", "sum", RooArgList(hist1,hist2), RooArgList(coe));
  RooAddPdf sum("sum", "sum", RooArgList(histpdf1,histpdf2), coe);
  RooPlot* frame3 = x.frame(Title("sum pdf"),Bins(100)) ;
  sum.plotOn(frame3) ;  

  TCanvas* c = new TCanvas("rf706_histpdf","rf706_histpdf",800,600) ;  
  c->Divide(2,2) ;
  c->cd(1) ; gPad->SetLeftMargin(0.15) ; frame1->GetYaxis()->SetTitleOffset(1.4) ; frame1->Draw() ;
  c->cd(2) ; gPad->SetLeftMargin(0.15) ; frame2->GetYaxis()->SetTitleOffset(1.8) ; frame2->Draw() ;
  c->cd(3) ; gPad->SetLeftMargin(0.15) ; frame3->GetYaxis()->SetTitleOffset(1.8) ; frame3->Draw() ;
