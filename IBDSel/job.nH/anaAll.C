...
int anaHists(int siteNum,string dataVer)
{
    ...
    if(anaLiHe)
    {
        TCanvas* c = new TCanvas("c","c",1200,750) ; 
        c->Divide(3,2);
        ...
        for( int ihist=0 ; ihist<1 ; ihist++ )
        {
                ...
                //for( int j=0 ; j<1 ; j++ )
            {
                ...
                eventTotalNum[j]=lh[ihist][j]->Integral(lh[ihist][j]->FindBin(0),lh[ihist][j]->FindBin(40));
                lh[ihist][j]->Rebin(10);
            ...
            for( int j=0 ; j<6 ; j++ )
            {
                ...
                RateMuon0[j]=NumMuon0[j]/totalDaqtime;
                ...
                std::cout<<"  R "<<minindex[j]*0.025<<"   n98["<<minindex[j]<<"] "<<n98fit[j]<<"   in98["<<minindex[j]<<"] "<< in98fit[j]<<"   fitRateMu["<<minindex[j]<<"] "<<rateMufit[j]<<"  realRateMu "<<RateMuon[j]<<"   NumIbd "<<NumIbd[j]<<" RealTotalNum "<<eventTotalNum[j]<<endl;
            }
    ...
    {
        double effOfMulti=0.;
        if(totalTime[i]!=0) effTime[i]/totalTime[i];
        double effOfMulti0=0.;
        if(totalTime0[i]!=0) effTime0[i]/totalTime0[i];
        if( (site=="EH3"||site=="EH2")&&i==(ADNumOfSite[siteNum-1]-1) )
        {
            std::cout<<"totalTimeSpecical  : "<<totalTimeSpecical<<endl;
            t_result[2][i+ii]=totalTimeSpecical/(3600*24);
            if(totalTimeSpecical!=0) t_result[3][i+ii]=totalTime0[i]/totalTimeSpecical;
        }else
        {
            t_result[2][i+ii]=totalTime[4]/(3600*24);

            if(totalTime[i]!=0) t_result[3][i+ii]=totalTime0[i]/totalTime[4];
        }
        std::cout<<"effTime0[i]  : "<<effTime0[i]<<endl;
        std::cout<<"totalTime0[i]  : "<<totalTime0[i]<<endl;
        if(totalTime0[i]!=0) t_result[4][i+ii]=effTime0[i]/totalTime0[i];
        ...
        } else
        {
            ...
            if(totalTime0[i]!=0) t_result[8][i+ii]=((NumAccBkg0[i]*sqrt(1/(double)totalN0[i]+(0.01*0.01)))/totalTime0[i])*24*3600;
            t_result[10][i+ii]=NumAccBkg0[i]*sqrt(1/(double)totalN0[i]+(0.01*0.01));
            if(totalTime0[i]!=0) t_result[12][i+ii]=(((NumAccBkg0[i]*sqrt(1/(double)totalN0[i]+(0.01*0.01))))/totalTime0[i])*24*3600;
        }
        if(totalTime0[i]!=0) t_result[7][i+ii]=(NumAccBkg0[i]/totalTime0[i])*24*3600;
        t_result[9][i+ii]=NumAccBkg0_600[i];
        if(totalTime0[i]!=0) t_result[11][i+ii]=(NumAccBkg0_600[i]/totalTime0[i])*24*3600;
        ...
        t_result[27][i+ii]=0;
        if(totalTime[i]!=0) t_result[28][i+ii]=totalTime[i]/totalTime[4];
        if(totalTime0[i]!=0&&totalTime[i]!=0) t_result[29][i+ii]=(totalTime[i]/totalTime[4]-totalTime0[i]/totalTime[4])/(totalTime0[i]/totalTime[4]);
        if(totalTime[i]!=0) t_result[30][i+ii]=effTime[i]/totalTime[i];
        ...
        } else
        {
            ...
            if(totalTime[i]!=0) t_result[36][i+ii]=((NumAccBkg[i]*sqrt(1/(double)totalN[i]+(0.01*0.01)))/totalTime[i])*24*3600;
            t_result[39][i+ii]=NumAccBkg[i]*sqrt(1/(double)totalN[i]+(0.01*0.01));
            if(totalTime[i]!=0) t_result[42][i+ii]=(NumAccBkg[i]*sqrt(1/(double)totalN[i]+(0.01*0.01))/totalTime[i])*24*3600;
        }
        if( NumAccBkg0[i]==0 )
        {
            t_result[34][i+ii]=-0;
        ...
        } else
        {
            ...
            if(totalTime0[i]!=0&&totalTime[i]!=0) t_result[37][i+ii]=-(NumAccBkg0[i]/totalTime0[i]-NumAccBkg[i]/totalTime[i])/(NumAccBkg0[i]/totalTime0[i]);
            t_result[40][i+ii]=-(NumAccBkg0_600[i]-NumAccBkg_600[i])/NumAccBkg0_600[i];
            if(totalTime0[i]!=0&&totalTime[i]!=0) t_result[43][i+ii]=-(NumAccBkg0_600[i]/totalTime0[i]-NumAccBkg_600[i]/totalTime[i])/(NumAccBkg0_600[i]/totalTime0[i]);
        }
        if(totalTime[i]!=0) t_result[35][i+ii]=(NumAccBkg[i]/totalTime[i])*24*3600;
        t_result[38][i+ii]=NumAccBkg_600[i];
        if(totalTime[i]!=0) t_result[41][i+ii]=(NumAccBkg_600[i]/totalTime[i])*24*3600;
    }
    ...
    if(anamuon)
    {
        ...
        for( int i=0 ; i<7 ; i++ )
        {
            if(totalTime[i]!=0) std::cout<<i+1<<". MuonNum :"<<totalNum[i]<<" Rate : "<<totalNum[i]/totalTime[4] <<" HZ"<<endl;
        }
        std::cout<<""<<endl;
    }
    for( int i=0 ; i<ADNumOfSite[siteNum-1] ; i++ )
    {
        if(totalTime[i]!=0) std::cout<<"Total AD"<<i+1<<"LiveTime                         : "<<totalTime[i]/(24*3600)<<" ("<<(totalTime[i] -totalTime0[i] )/totalTime0[i] <<") day"<<endl;
        if(totalTime0[i]!=0) std::cout<<"Total AD"<<i+1<<"LiveTime(without rpc) : "<<totalTime0[i]/(24*3600)<<" day"<<endl;
    }
