...
void Ibd::Begin(TTree * /*tree*/)
{
    ...
    std::cout<<"daqtime0[1][0]  : "<<daqtime0[1][0]<<endl;
    std::cout<<"daqtime0[1][1]  : "<<daqtime0[1][1]<<endl;
    std::cout<<"t_result[2][2]  : "<<t_result[2][2]<<endl;
    std::cout<<"t_result[2][3]  : "<<t_result[2][3]<<endl;
    ...
    tNum2=0;
    dofit4fn=0;
    string nameStr=Form("%s/%slivetime_%s.root",dataVer.c_str(),option.Data(),dataVer.c_str());
    TFile* f4fn= new TFile(nameStr.c_str());
    if( f4fn->IsZombie() )
    {
        std::cout<<"Error :  can't open T:"<<nameStr<<endl;
    }
    TH1F* h4fn[4];

    TH1F* h4fnDaq=(TH1F*)f4fn->Get("DaqTime");
    //h4fnDaq->Draw();
    if( !h4fnDaq )
    {
        std::cout<<"Error :  can't open H:DaqTime"<<endl;
    }
    double timeThr[8]={1324725000,1334087000,1343450000,1350630000,1359462500,1368294500,1377126500,1385960000};
    int binThr[8];
    for( int i=0 ; i<8 ; i++ )
    {
        binThr[i]=h4fnDaq->FindBin(timeThr[i]);
        binLowEdge[i]=h4fnDaq->GetBinLowEdge(binThr[i]);
    }

    double totalLiveTime[7]={0.};
    for( int i=0 ; i<ADNum ; i++ )
    {
        nameStr=Form("AD%iLiveTime0",i+1);
        h4fn[i]=(TH1F*)f4fn->Get(nameStr.c_str());
        if( !h4fn[i] )
        {
            std::cout<<"Error :  can't open H:"<<nameStr<<endl;
        }
        for( int j=0 ; j<7 ; j++ )
        {
            for( int k=binThr[j] ; k<binThr[j+1] ; k++ )
            {
                totalLiveTime[j]+=h4fn[i]->GetBinContent(k);
            }
    ...
    h4fnRate= new TH1F("h4fnRate","h4fnRate",7,binLowEdge);
    //h4fnRate->Draw();
    for( int i=0 ; i<7 ; i++ )
    {
        h4fnRate->SetBinContent(i+1,totalLiveTime[i]);
        std::cout<<"i  : "<<h4fnRate->GetBinContent(i+1)<<endl;
    }
    ...
    for( int i=0 ; i<7 ; i++ )
    {
        h4fnFit[i]=new TH1F(Form("h4fnFit%i",i+1),"h4fnFit",nBins, EBins);
    }
...
Bool_t Ibd::Process(Long64_t entry)
{
    ...
    if( isIbd==0 )
    {
        ...
        for( int i=0 ; i<7 ; i++ )
        {
            if( delayedTrigTimeSec>binLowEdge[i]&&delayedTrigTimeSec<binLowEdge[i+1] )
            {
                h4fnFit[i]->Fill(energy[0],wt);
                break;
            }
        }
...
void Ibd::Terminate()
{
    ...
    if( dofit4fn )
    {
        ...
        for( int i=0 ; i<7 ; i++ )
        {
            ...
            /*
               for( int j=binlow ; j<=binup ; j++ )
               {
               bincenter=h4fnFit[i]->GetBinCenter(j);
               NFn10+=(par10[0]+par10[1]*bincenter)/BinWt[j];
               iNFnsquare10+=(bincenter*bincenter*ipar10[1]*ipar10[1]+ipar10[0]*ipar10[0])/(BinWt[j]*BinWt[j]);
               }
             */
             ...
             */
            NFn00=h4fnFit[i]->Integral(h4fnFit[i]->FindBin(12),h4fnFit[i]->FindBin(100),"width")*(12-0.7)/(100-12);
            iNFnsquare00=sqrt(h4fnFit[i]->Integral(h4fnFit[i]->FindBin(12),h4fnFit[i]->FindBin(100),"width"))*(12-0.7)/(100-12);
            double totalNumber=(NFn00+NFn10)/2;
            double totalNumberErr=(sqrt((iNFnsquare00+iNFnsquare10)/4+(NFn10-NFn00)*(NFn10-NFn00)));
            double totallivetime=(h4fnRate->GetBinContent(i+1)/24/3600);
            double totalRate=(NFn00+NFn10)/2/totallivetime;
            double totalRateErr=(sqrt((iNFnsquare00+iNFnsquare10)/4+(NFn10-NFn00)*(NFn10-NFn00)))/totallivetime;
            h4fnRate->SetBinContent(i+1,totalRate);
            h4fnRate->SetBinError(i+1,totalRateErr);
            std::cout<<"	number  : "<< totalNumber<<" rate : "<<totalRate <<" /day " <<endl;

            leg->AddEntry((TObject*)0,Form("t%i : %5.1f fn num: %0.2f+-%0.2f",i,totallivetime,totalNumber,totalNumberErr),"");
            std::cout<<"i "<<h4fnRate->GetBinContent(i+1)<<endl;
        }
        TCanvas* c1=new TCanvas("c1","c1",600,400) ;
        h4fnRate->GetXaxis()->SetTimeDisplay(1);
        h4fnRate->GetXaxis()->SetTimeFormat("%d/%m/%y%F1970-01-01 00:00:00");
        h4fnRate->Draw();
        leg->Draw("same");
        c1->Write();
    }
    ...
    iNFnsquare1=(ipar1[0]*2+12.7*ipar1[1])*(12-0.7)/2;
