{
  ...
  const int nhist = 6;
  int nrebin = 10;
  TString filename = "/publicfs/dyb/data/userdata/zhangfh/IbdAna/ana/HeLiAna/P12Etot/EH1/mergedHist.root";
  TString Site = "EH1";
  TString outputname="EH1red.root";
  int nAD=2;
  ...
  for(int ihist=1; ihist<=nhist; ihist++){
    ...
    TString str = "zhangfh/time2LastMuonRed"; 
    /*
    switch(ihist){
    ...
    */
    ...
    for(int i=0; i<41; i++) {
      ...
      cout << "1" << endl;
      fitres = sum->fitTo((*data), Save(),PrintLevel(-1));
      ...
      if(minNLL[i]<glbminNLL) { glbminNLL = minNLL[i]; minIndex = i;std::cout<<"now is  "<<i<<endl; }
    }
    for(int i=0; i<41; i++) {
