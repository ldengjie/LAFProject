const int m_NAD=1;
TString m_finputname="/publicfs/dyb/data/userdata/zhangfh/IbdAna/ana/P12C-bak/merged/mergedHistEH2.root";
TString m_foutputname="fn_EH2.root";
...
void fastNeu2(){
  ...
  TH1D* promptE=new TH1D("promptE","",nBins,eBins);
  ...
  for(Int_t i=1;i<=m_NAD;i++){
    ...
    promptE->Add((TH1D*)gDirectory->Get(histName));
  }
  ...
  TH1D* promptEcopy=(TH1D*)promptE->Clone();
