...
vector<double>  DsG4GdCaptureGammas::GetEnergy ()
{
  ...
  goon:
    ...
    if(gamma_energy[gamma_num]<0.8) {
        ...
        if(energy_tmp>7.4) {
            G4cout << "energy_tmp>7.4!" << gamma_energy[gamma_num] << ", " << gamma_energy[gamma_num-1] << G4endl;
            goto ok;
        }
    ...
    //// add more constraints on gamma number
    //if(gamma_num>=5) {
    //    energy_sum = 0.0;
    //    gamma_num = 0;
    //    for(int i=0; i<100; i++) { gamma_energy[i]=0; }
  ...
  ok:
    ...
    //if(gamma_num>=5) G4cout << "gamma_num>=5!" << gamma_num << G4endl;
